...
function Paddle:init(x, y, width, height, behavior)
    self.x = x
    self.y = y
    self.width = width
    self.height = height
...
function Paddle:ai(ball, gameState)
    if gameState == 'play' then
            if self.x > VIRTUAL_WIDTH / 2 then
                if ball.y > self.y then
                    self.dy = PADDLE_SPEED * 0.7
                    if (ball.x * VIRTUAL_WIDTH) / WINDOW_WIDTH > VIRTUAL_WIDTH / 2 then
                        self.dy = self.dy * 1.2
                    end
                end
                ...
                if ball.y >= self.y and ball.y <= self.y + self.width then
                       self.dy = 0
                end
            else
                if ball.y > self.y then
